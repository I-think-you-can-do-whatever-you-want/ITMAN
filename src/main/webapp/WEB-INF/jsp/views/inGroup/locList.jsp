...
<body>
	<div id="contents">
		<div class="tit_search">
			<h2>위치 관리</h2>
			<form id="searchForm" action="${pageContext.request.contextPath}/assetLocationList.do" method="get" name="searchForm" onsubmit="this.page.value=1; this.range.value=1;">
        ...
        <?=$test?>
		<!-- 글쓰기 버튼-->
		<p class="addContent"><a onclick="window.open('/asset/contWriteAssetLocation.do', '위치 추가 팝업', 'width=500, height=500, status=no,toolbar=no,scrollbars=no')" href="#" class="edit"><span></span><span></span><span></span></a></p>
		<div class="Basic">
			<ul class="adminList">
                <li class="tit">
					<p class="num">코드번호</p>
					...
					<c:forEach var="location" items="${resultList}">
						<li>
							...
							<p class="editDel" style="padding: 0">
								<a onclick="window.open('/editLocation.do?locIdx=${location.locIdx}', 'EditPopUp', 'width=500, height=500, status=no,toolbar=no,scrollbars=no')" href="#" class="edit">수정</a>
								<a onclick="window.open('/confirmLocationDel.do?locIdx=${location.locIdx}', 'EditPopUp', 'width=500, height=350, status=no,toolbar=no,scrollbars=no')" href="#" class="del">삭제</a>
							</p>
						</li>
					</c:forEach>
				</c:if>
...
<jsp:include page="${pageContext.request.contextPath}/WEB-INF/jsp/_inc/footer.jsp" />
	<script>
		...
		//처음 버튼 이벤트
		function fn_maxPrev() {
			var url = "${pageContext.request.contextPath}/assetLocationList.do";
			url = url + "?page=" + 1;
			url = url + "&range=" + 1;
			location.href = url;	}
		//이전 버튼 이벤트
