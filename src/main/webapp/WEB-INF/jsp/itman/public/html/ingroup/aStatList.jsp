...
<body>
	...
	<div id="contents">
		<div class="tit_search">
			<h2>자산 상태 관리</h2>
			<form id="searchForm" method="get" action="${pageContext.request.contextPath}/assetStateList.do" onsubmit="this.page.value=1; this.range.value=1;">
		...
		<!-- 글쓰기 버튼-->
		<p class="addContent"><a href="#" onclick="window.open('/asset/contWriteAssetState.do', 'EditPopUp', 'width=500, height=400, status=no,toolbar=no,scrollbars=no')" class="edit">><span></span><span></span><span></span></a></p>
					...
					<c:forEach var="state" items="${resultList}">
						<li>
							...
							<p class="editDel" style="padding: 0">
								<a href="#" onclick="window.open('/editAssetState.do?staIdx=${state.staIdx}', 'EditPopUp', 'width=500, height=350, status=no,toolbar=no,scrollbars=no')" class="edit">수정</a>
								<a href="#" onclick="window.open('/confirmAssetStateDel.do?staIdx=${state.staIdx}', 'EditPopUp', 'width=500, height=350, status=no,toolbar=no,scrollbars=no')" class="del">삭제</a>
							</p>
						</li>
					</c:forEach>
				</c:if>
...
<jsp:include page="${pageContext.request.contextPath}/WEB-INF/jsp/itman/_inc/footer.jsp" />
	<script>
		...
		//처음 버튼 이벤트
		function fn_maxPrev() {
			var url = "${pageContext.request.contextPath}/assetStateList.do";
			url = url + "?page=" + 1;
			url = url + "&range=" + 1;
			location.href = url;	}
		//이전 버튼 이벤트
