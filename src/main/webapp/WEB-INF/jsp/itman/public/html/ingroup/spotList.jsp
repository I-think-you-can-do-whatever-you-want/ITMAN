...
<body>
	...
	<div id="contents">
		...
		<h2>직위 관리</h2>
			<form  id="searchForm" name="searchForm" method="get" action="${pageContext.request.contextPath}/spotList.do" onsubmit="this.page.value=1; this.range.value=1;">
		...
		<!-- 글쓰기 버튼-->
		<p class="addContent">
		    <a href="#" onclick="window.open('/positionWrite.do' ,'직위 추가 팝업' ,'width=500, height=350, status=no,toolbar=no,scrollbars=no' )" class="edit">><span></span><span></span><span></span></a></p>
				...
				<c:if test="${!empty resultList}">
                    ...
                    <li>
                        ...
                        <p class="editDel" style="padding:0;">
                            <a href="#" onclick="window.open('/positionEdit.do?posIdx=${position.posIdx}', 'EditPopUp', 'width=500, height=350, status=no,toolbar=no,scrollbars=no')" class="edit">수정</a>
                            <a href="#" onclick="window.open('/confirmPositionDel.do?posIdx=${position.posIdx}', 'EditPopUp', 'width=500, height=350, status=no,toolbar=no,scrollbars=no')" class="del">삭제</a>
                        </p>
                    </li>
					</c:forEach>
				</c:if>
	...
	<script>
		...
		//처음 버튼 이벤트
		function fn_maxPrev() {
			var url = "${pageContext.request.contextPath}/spotList.do";
			url = url + "?page=" + 1;
			url = url + "&range=" + 1;
			location.href = url;	}
		//이전 버튼 이벤트
